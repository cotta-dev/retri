...
func TestResolveLogEncodingPriority(t *testing.T) {
	rh := ResolvedHost{
		HostConfig: HostConfig{CommonFields: CommonFields{LogEncoding: "windows-1252"}},
		GroupConfigs: []GroupConfig{
			{CommonFields: CommonFields{LogEncoding: "euc-jp"}},
		},
		DeviceConfig: DeviceConfig{CommonFields: CommonFields{LogEncoding: "shift_jis"}},
	}
	defaults := GlobalOptions{CommonFields: CommonFields{LogEncoding: "utf-8"}}

	if got, want := ResolveLogEncoding(rh, defaults, ""), "windows-1252"; got != want {
		t.Fatalf("ResolveLogEncoding() = %q, want host value %q", got, want)
	}
	if got, want := ResolveLogEncoding(rh, defaults, "gb18030"), "gb18030"; got != want {
		t.Fatalf("ResolveLogEncoding() = %q, want CLI value %q", got, want)
	}
}

func TestResolveLogEncodingEnvironmentFallback(t *testing.T) {
	t.Setenv("RETRI_LOG_ENCODING", "euc-kr")
	if got, want := ResolveLogEncoding(ResolvedHost{}, GlobalOptions{}, ""), "euc-kr"; got != want {
		t.Fatalf("ResolveLogEncoding() = %q, want environment fallback %q", got, want)
	}
}
