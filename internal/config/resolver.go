...
// environment fallback < defaults < groups < device_types < hosts < CLI.
// Environment expansion inside a selected config value happens before the
...
// ResolveLogEncoding resolves log_encoding using the CommonFields priority
// chain. RETRI_LOG_ENCODING is used only when no config value is set, and a
// CLI value always wins.
func ResolveLogEncoding(rh ResolvedHost, defaults GlobalOptions, cliValue string) string {
	return resolveCommonString(rh, defaults, func(fp FieldProvider) string {
		return fp.Common().LogEncoding
	}, "RETRI_LOG_ENCODING", cliValue)
}

func resolveCommonString(rh ResolvedHost, defaults GlobalOptions, getField func(FieldProvider) string, envKey, cliVal string) string {
	...
	if v := getField(&rh.DeviceConfig); v != "" {
		val = v
	}
	if v := getField(&rh.HostConfig); v != "" {
		val = v
	}
